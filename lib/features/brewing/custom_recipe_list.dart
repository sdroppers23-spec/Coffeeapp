...
final brewingSelectedIdsProvider = NotifierProvider<BrewingSelectionNotifier, Set<String>>(() {
  ...
  Widget build(BuildContext context) {
    ...
    return recipesAsync.when(
      ...
      data: (recipes) {
        ...
        final espressoRecipes = filtered.where((r) => isEspresso(r.methodKey, r.recipeType)).toList();
        final filterRecipes = filtered.where((r) => !isEspresso(r.methodKey, r.recipeType)).toList();
                      ...
                      child: CustomRecipeCard(
                        ...
                        onTap: () {
                          if (_isSelectionMode) {
                            ...
                            ref.read(brewingSelectedIdsProvider.notifier).updateState(next);
                          }
                        },
                        onLongPress: () {
                          if (!_isSelectionMode) {
                          ...
                          child: Row(
                            children: [
                              ...
                              _buildSelectionAction(
                                Icons.delete_outline_rounded,
                                const Color(0xFFEF5350),
                                selectedIds.isEmpty ? null : _deleteSelectedBatch,
                              ),
                              const SizedBox(width: 8),
                              _buildSelectionAction(
                                Icons.close_rounded,
