...
class _TemplateChip extends ConsumerWidget {
                    ...
                    child: Column(
                      children: [
                        _buildGlassSection(
                          ...
                          onBaseOpacityChanged: (val) => notifier.updateConfig(config.copyWith(baseOpacity: val)),
                        ),
                        ...
                        _buildGlassSection(
                          ...
                          baseOpacity: config.navBarBaseOpacity,
                          onOpacityChanged: (val) => notifier.updateNavBar(val, config.navBarBlur, config.navBarBaseOpacity),
                          onBlurChanged: (val) => notifier.updateNavBar(config.navBarOpacity, val, config.navBarBaseOpacity),
                          onBaseOpacityChanged: (val) => notifier.updateNavBar(config.navBarOpacity, config.navBarBlur, val),
                        ),
                        _buildGlassSection(
                          title: 'Картка смаків',
                          opacity: config.flavorCardOpacity,
                          blur: config.flavorCardBlur,
                          baseOpacity: config.flavorCardBaseOpacity,
                          onOpacityChanged: (val) => notifier.updateFlavorCard(val, config.flavorCardBlur, config.flavorCardBaseOpacity),
                          onBlurChanged: (val) => notifier.updateFlavorCard(config.flavorCardOpacity, val, config.flavorCardBaseOpacity),
                          onBaseOpacityChanged: (val) => notifier.updateFlavorCard(config.flavorCardOpacity, config.flavorCardBlur, val),
                        ),
                        _buildGlassSection(
                          title: 'Діалоги профілю',
                          opacity: config.profileOpacity,
                          blur: config.profileBlur,
                          baseOpacity: config.profileBaseOpacity,
                          onOpacityChanged: (val) => notifier.updateProfile(val, config.profileBlur, config.profileBaseOpacity),
                          onBlurChanged: (val) => notifier.updateProfile(config.profileOpacity, val, config.profileBaseOpacity),
                          onBaseOpacityChanged: (val) => notifier.updateProfile(config.profileOpacity, config.profileBlur, val),
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildGlassSection({
    ...
    required ValueChanged<double> onBaseOpacityChanged,
  }) {
    return Theme(
      ...
      child: ExpansionTile(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Прозорість основи', style: TextStyle(color: Colors.white70, fontSize: 13)),
              Text(baseOpacity.toStringAsFixed(2), style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)),
            ],
          ),
          Slider(
            value: baseOpacity,
            ...
            onChanged: onBaseOpacityChanged,
          ),
          const SizedBox(height: 8),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('Прозорість тінту', style: TextStyle(color: Colors.white70, fontSize: 13)),
